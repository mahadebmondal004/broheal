...
const UserLogin = () => {
    ...
    const handlePasswordLogin = async (e) => {
        ...
        setLoading(true);
        try {
            const response = await api.post('/auth/login', { phone, password, role: 'user' });
            const { user, accessToken, refreshToken } = response.data;

            if (user.role !== 'user') {
                toast.error('This portal is for Users only.');
                return;
            }

            login(user, accessToken, refreshToken);
            toast.success('Welcome back to BroHeal!');
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden bg-slate-900">
            ...
            <div className="absolute top-[40%] left-[60%] w-[20%] h-[20%] bg-cyan-500/20 rounded-full blur-[80px]" />
            ...
            <div className="relative w-full max-w-md perspective-1000">
                {/* Main Card */}
                <div className="relative backdrop-blur-xl bg-white/10 border border-white/20 rounded-3xl shadow-2xl p-8 overflow-hidden group hover:shadow-blue-500/10 transition-all duration-500">
                    ...
                    <div className="text-center mb-8 relative z-10">
                        <div className="w-20 h-20 mx-auto bg-gradient-to-br from-blue-500 to-cyan-400 rounded-2xl shadow-lg flex items-center justify-center mb-4 transform group-hover:scale-105 transition-transform duration-300">
                            <img src={logoUrl} alt="Bro Heal" className="w-16 h-16 object-contain drop-shadow-md brightness-0 invert" />
                        </div>
                        <h1 className="text-3xl font-bold text-white mb-2 tracking-tight">User Login</h1>
                        <p className="text-blue-200 text-sm font-medium">Your personal wellness portal</p>
                    </div>
                    ...
                    <form onSubmit={handlePasswordLogin} className="space-y-6 relative z-10">
                        ...
                        <div className="space-y-2 group/input">
                            <label className={`text-xs font-semibold uppercase tracking-wider transition-colors duration-300 ${focusedField === 'phone' ? 'text-blue-400' : 'text-slate-400'}`}>
                                Phone Number
                            </label>
                            <div className={`relative flex items-center bg-slate-800/50 border rounded-xl transition-all duration-300 ${focusedField === 'phone' || (phone && isValidPhone(phone))
                                ...
                                />
                                <div className="pr-4">
                                    <Phone className={`w-5 h-5 transition-colors duration-300 ${focusedField === 'phone' ? 'text-blue-400' : 'text-slate-500'}`} />
                                </div>
                            </div>
                        </div>
                        ...
                        {/* Password Input */}
                        <div className="space-y-2 group/input">
                            <label className={`text-xs font-semibold uppercase tracking-wider transition-colors duration-300 ${focusedField === 'password' ? 'text-blue-400' : 'text-slate-400'}`}>
                                Password
                            </label>
                            <div className={`relative flex items-center bg-slate-800/50 border rounded-xl transition-all duration-300 ${focusedField === 'password'
                                ...
                                />
                                <div className="pr-4">
                                    <KeyRound className={`w-5 h-5 transition-colors duration-300 ${focusedField === 'password' ? 'text-blue-400' : 'text-slate-500'}`} />
                                </div>
                            </div>
                        ...
                        <button
                            ...
                            className="w-full relative group overflow-hidden bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-bold py-4 rounded-xl shadow-lg hover:shadow-cyan-500/25 transition-all duration-300 transform hover:-translate-y-1 hover:scale-[1.02] disabled:opacity-50 disabled:transform-none disabled:shadow-none"
                        >
